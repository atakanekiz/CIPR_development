...
# Things to check
## reference log transformation. What happens if the expression val is zero in a column?
...
server <- function(input, output){
  ...
  user_data <- reactive({
    ...
    if(input$comp_method == "logFC SoM"){
      ...
      if(is.null(inFile) & input$example_data == T){
        ...
        validate(
          need(
            ...
            "Formatting error: Make sure your dataset contains at least three columns named 'logfc', 'gene', and 'cluster' (Capitalization of the column names is not important, but duplicate names are not allowed)"
          )
        )
